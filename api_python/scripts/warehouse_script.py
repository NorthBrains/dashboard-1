...
def create_producer():
    conf = {'bootstrap.servers': "172.30.0.6:9092, 172.30.0.7:9092, 172.30.0.8:9092"}
...
def update_data(df):
    ...
    for (product, location), group in df.groupby(['Product Name', 'Location']):
        ...
        producer.produce('warehouse', key=str(last_row['Product ID']), value=json.dumps(enriched_product), callback=delivery_report)
        producer.poll(0)
