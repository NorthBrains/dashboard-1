...
if [ "$COUNT" -gt "$RECORD_LIMIT" ]; then
  ...
  echo "$(date): Deleted $DELETE_COUNT records from $TABLE, keeping the latest $RECORD_LIMIT records." >> /initdb/cron_del_sales_data.sh
else
  echo "$(date): No records deleted, only $COUNT records present in $TABLE" >> /initdb/cron_del_sales_data.sh
fi