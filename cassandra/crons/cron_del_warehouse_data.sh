...
TABLE="company_one.warehouse_data"
RECORD_LIMIT=500
...
if [ "$COUNT" -gt "$RECORD_LIMIT" ]; then
  ...
  echo "$(date): Deleted $DELETE_COUNT records from $TABLE, keeping the latest $RECORD_LIMIT records." >> /crons/cron_del_warehouse_data.sh
else
  echo "$(date): No records deleted, only $COUNT records present in $TABLE" >> /crons/cron_del_warehouse_data.sh
fi